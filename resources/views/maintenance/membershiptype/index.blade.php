...
@section('content')
    ...
    @if(Session::has('membershipTypeStatus'))
        <div class="row">
            <div class="col-sm-12 alert alert-info text-center">
                {{session('membershipTypeStatus')}}
            </div>
        </div>
    @endif
    ...
    @if($membershipTypes)
        @foreach($membershipTypes as $membershipType)
            <div class="row pt-1 pb-1 detail-borders">
                <div class="col-sm-1 text-center pt-1">{{$membershipType->id}}</div>
                <div class="col-sm-4 pt-1">{{$membershipType->description}}</div>
                <div class="col-sm-3 text-center pt-1">
                    {{$membershipType->updated_at->diffForHumans()}}
                </div>
                <div class="col-sm-2 text-center">
                    <a href="{{route('membershiptype.edit', $membershipType->id)}}" class="btn btn-primary">
                        <span class="fa fw fa-edit"></span> Edit
                    </a>
                </div>
                <div class="col-sm-2 text-center">
                    <a href="{{route('membershiptype.show', $membershipType->id)}}" class="btn btn-primary">
